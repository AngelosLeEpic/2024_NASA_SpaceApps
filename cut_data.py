...
def get_seismic_events(df: pd.DataFrame, velocity_col: str) -> pd.DataFrame:
    ...
    df = df.reset_index()
    df["STA"] = df[velocity_col].rolling(window=1000).mean() # make the short term average
    # add median to velocitys
    median = df[velocity_col].median() * 4
    df[velocity_col + " and median"] = df[velocity_col].apply(lambda x: x+median)
    df["LTA"] = df[velocity_col + " and median"].rolling(window=20000).mean() # make the long term average
